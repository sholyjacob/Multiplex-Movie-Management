...
public class SocketActor extends AbstractActor {
    ...
    private CompletionStage<HttpResponse> callRestApi(String name){
        ...
        return Http.get(getContext().getSystem()).singleRequest(HttpRequest.create("http://localhost:9000/movie/searchByName" + name));
    }

    // method to consume httpResponse
    private CompletionStage<MovieModel> consumeHttpResponse(HttpResponse httpResponse){
        ...
        return Jackson.unmarshaller(MovieModel.class)
                .unmarshal(httpResponse.entity(), materializer)
                .thenApply(movieModel -> {
